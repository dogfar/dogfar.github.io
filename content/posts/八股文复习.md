...
11. CPU缓存一致性（单CPU：写回、写直达）、MESI协议（多CPU）；
...
在编译时运算表达式可以改善执行时间，因为需要执行的代码少了，编译器可做额外的优化。 [C++](https://c.biancheng.net/cplus/) 中的关键字 constexpr（constant expression 的简称）用于声明编译时常量对象和函数。
